...
def test_queue_processes_requests_in_fifo_order() -> None:
    ...
    first = RestorationRequest(
        40,
        "Polish cracked frame"
    )

    second = RestorationRequest(
        20,
        "Oil the wing gears"
    )

    third = RestorationRequest(
        60,
        "Flatten folded corner"
    )
...
def test_stack_push_and_peek_do_not_remove_item() -> None:
    ...
    stack.push_action(
        "Added Cursed Mirror to archive"
    )
...
def test_sort_artifacts_by_age_descending() -> None:
    ...
    sorted_artifacts = sort_artifacts_by_age(
        artifacts,
        descending=True
    )
...
def test_demo_museum_night_runs_and_prints_key_sections(
    capsys
) -> None:
    demo_museum_night()
